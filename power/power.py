...
parser = argparse.ArgumentParser(
    description="Calculate total wake time from log with optional debug output."
)
...
def calculate_wake_times():
    ...
    # Calculate and print total wake time for each day
    wake_times_summed = {}
    ...
    # Calculate and print total wake time for the last 7 days
    today = datetime.now().date()
    seven_days_ago = today - timedelta(days=7)
    last_7_days_total = timedelta()
    for date, durations in wake_periods.items():
        if seven_days_ago <= date <= today:
            last_7_days_total += sum(durations, timedelta())
    total_seconds = last_7_days_total.total_seconds()
    hours = total_seconds // 3600
    minutes = (total_seconds % 3600) // 60
    print(f"Total Wake Time for the Last 7 Days: {int(hours)}h {int(minutes)}m")
